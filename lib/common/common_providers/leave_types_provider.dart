...
class LeaveTypesProvider extends ChangeNotifier {
  ...
  List<LeaveType> leaveTypesForFilter = [];
  ...
  Future<void> getLeaveBalances(BuildContext context) async {
    ...
    await LeaveServices.getLeaveTypes(
      authToken,
    ).then((result) {
      try {
        if (result != null) {
  ...
  _onSuccess(APIResponse response) {
    try {
      List<LeaveType> leaveBalanceList = response.data['result']
          .map((leaveBalance) => LeaveType.fromJson(leaveBalance))
          .toList()
          .cast<LeaveType>();

      leaveTypes.addAll(leaveBalanceList);

      if (leaveTypes.isNotEmpty) {
        leaveTypesForFilter.add(
          LeaveType(typeId: 0, leaveTypeName: 'None', leaveTypeCode: ''),
        );
        leaveTypesForFilter.addAll(leaveTypes);
